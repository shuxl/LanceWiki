...
+ jdk17的AQS代码详解 https://blog.csdn.net/weixin_49561445/article/details/120598020
+ [ ] 不理解 Condition的逻辑，这篇帖子也是不理解 ： https://blog.csdn.net/jiankunking/article/details/83443351
- 一句话概括AQS：
	- JUC包中多个组件的底层实现，Lock、CountDownLatch、Semaphore都用了AQS
	- AQS提供了两种锁的机制，排它锁和共享锁
		- 排它锁：多个线程竞争一个共享资源。比如Lock中的ReentrantLock重入锁，就是用到了AQS的一个排它锁的功能
		- 共享锁：也称为读锁，就是同一个时刻运行多个线程获得这样一个锁的资源，比如CountDownlatch、Semaphore都用到了AQS中的共享锁的功能
	- AQS作为互斥锁，需要解决三个核心设计
		- 第一个，互斥变量的设计，以及如何保证多线程同时更新互斥变量的时候线程的安全性
		- 第二个，未竞争到锁的资源的线程的等待，竞争到锁的资源释放锁之后的唤醒
		- 第三个，锁竞争的公平性和非公平性
		- AQS采用了一个int类型的互斥变量`state`，用来记录锁的竞争的状态。0标识没有任何线程竞争锁资源，而大于等于1，表示已经有线程正在持有锁资源。
		- 一个线程在获取锁资源的时候，首先判断`state`是否等于0（也就是说它是无锁状态），如果是则更新为1，表示占有锁。这个过程中，如果又个线程同时去做这样一个操作，就会导致线程安全性问题。因此，AQS采用了CAS机制去保证state互斥变量更新的一个原子性。未获得到锁的线程，通过Unsafe类中的park方法，进行阻塞，把阻塞的线程按照先进先出的原则，去加入到一个双向链表的结构中，当获得锁的线程在释放锁之后，会从这样一个双向链表的头部，去唤醒下一个等待的线程，再去竞争锁。
		- 锁竞争的公平性和非公平性的问题，AQS的处理方式是，在竞争锁资源的时候，公平锁需要判断，双向链表中是否有阻塞的线程，如果有需要排队等待；而非公平锁的处理方式是，不管双向链表中是否有阻塞的线程，都回去尝试去更改互斥变量state去竞争锁，假设在一个临界点获得锁的线程释放锁，此时state等于0，当前线程抢占锁的时候，正好吧这个state修改成1，那么这个时候就表示它可以拿到锁，而这个过程是非公平的
## 0.1 带着BAT大厂的面试问题去理解
- 什么是AQS? 为什么它是核心?
...
# 2 AbstractQueuedSynchronizer数据结构
...
![](img/Pasted%20image%2020240718210922.png)
...
![](img/Pasted%20image%2020240718211754.png)
...
## 3.8 需要子类实现的方法 - isHeldExclusively()
该线程是否正在独占资源。只有用到condition才需要去实现它。

标准实现类各自的实现逻辑：
1. ThreadPoolExecutor.Work 实现逻辑为：
	return getState() != 0;
    
2. ReentrantLock和ReentrantReadWriteLock 实现逻辑为：
	return getExclusiveOwnerThread() == Thread.currentThread()

## 3.9 需要子类实现的方法 - tryAcquire(int)
独占方式。尝试获取资源，成功则返回true，失败则返回false。
一般情况下，都会去自己管理state变量，比如将state值设置为入参值。

标准实现类个字的实现逻辑
1. ReentrantLock.FairSync#tryAcquire
	1. 添加判断是否要进行非公平锁的竞争
	2. 以及实现可重入逻辑。可重入逻辑的意思就是同一个线程多次获取锁时，state 值增加
2. ReentrantLock.NonfairSync#tryAcquire
	1. 实现可重入逻辑的功能
3. ReentrantReadWriteLock.Sync#tryAcquire
	1. state逻辑的定制处理。比如将state的第16位设置为state值

## 3.10 需要子类实现的方法 - tryRelease(int)
独占方式。尝试释放资源，成功则返回true，失败则返回false
已知的java内部类的实现逻辑，都是在管理state和exclusiveOwnerThread，

标准实现类的几个方法：
1. ThreadPoolExecutor.Worker#tryRelease
	1. state设置为0，exclusiveOwnerThread设置为null
2. ReentrantLock.Sync#tryRelease
	1. state设置为 state - 入参，可重入逻辑的实现
3. ReentrantReadWriteLock.Sync#tryRelease
	1. 也是可重入逻辑的实现

## 3.11 需要子类实现的方法 - tryAcquireShared(int)
共享方式。尝试获取资源。负数表示失败；0表示成功，但没有剩余可用资源；正数表示成功，且有剩余资源。
不同类的实现风格有差异！

标准实现类的几个思路：
1. ReentrantReadWriteLock.Sync#tryAcquireShared
	1. 首先验证是否存在写线程占用共享资源，如果被占用，则返回-1
	2. 否则就尝试去获取读锁。这里读锁是讲state的前16个字节修改
2. 其它类暂不总结。。。。。
	1. 。。。。。

## 3.12 需要子类实现的方法 - tryReleaseShared(int)
共享方式。尝试释放资源，如果释放后允许唤醒后续等待结点返回true，否则返回false。


# 4 AbstractQueuedSynchronizer示例详解一
...
![](img/Pasted%20image%2020240718212830.png)
...
![](img/Pasted%20image%2020240718212835.png)
...
![](img/Pasted%20image%2020240718212852.png)
...
![](img/Pasted%20image%2020240718212858.png)

说明: t2线程执行lock.unlock后，最终达到的状态还是与之前的状态一样。

# 5 AbstractQueuedSynchronizer示例详解二
...
![](img/Pasted%20image%2020240718212910.png)
...
![](img/Pasted%20image%2020240718213005.png)

说明: 其中，head设置为包含c1线程的结点，c1继续运行。

- c1线程执行fullCondtion.signal，其方法调用顺序如下，只给出了主要的方法调用。

![](img/Pasted%20image%2020240718213013.png)
...
![](img/Pasted%20image%2020240718213018.png)

说明: 最终c2线程会获取锁资源，继续运行用户逻辑。

- c2线程执行emptyCondition.await，由前面的第七步分析，可知最终的状态如下。

![](img/Pasted%20image%2020240718213026.png)
...
![](img/Pasted%20image%2020240718213034.png)

- p2继续运行，执行emptyCondition.signal，根据第九步分析可知，最终到达的状态如下。

![](img/Pasted%20image%2020240718213052.png)
...
![](img/Pasted%20image%2020240718213057.png)
...
# 6 AbstractQueuedSynchronizer总结
