...
+ 压缩列表 - ZipList 
	+ 结构是什么？压缩性体现在哪里？
	+ 压缩后牺牲的哪些东西，带来的负面影响是啥
+ 跳表 - ZSkipList
	+ 重点：引入的目的是核心，然后它的结构以及动态变化的过程，最后是与平衡树的区别
## 0.1 底层数据结构引入

在对对象机制（redisObject）有了初步认识之后，我们便可以继续理解如下的底层数据结构部分：
![](img/Pasted%20image%2020240805221020.png)
...
![](img/Pasted%20image%2020240805222100.png)
...
先看下6.0中对应的源码和介绍
![](img/Pasted%20image%2020240805222258.png)

整个ziplist在内存中的存储格式如下：

![](img/Pasted%20image%2020240805222327.png)
...
- **先看下源码中相关介绍**
![](img/Pasted%20image%2020240805222508.png)
...
- 因为这些元素在内存中是连续的，所以 ziplist 在增加或删除元素时可能需要进行内存的重新分配和数据的移动，以保持连续性。这种设计使得 ziplist 在处理少量小数据时非常高效，但在存储大量数据时可能会带来性能开销。
...
![](img/Pasted%20image%2020240806082702.png)
...
![](img/Pasted%20image%2020240806083015.png)
...
![](img/Pasted%20image%2020240806083240.png)
...
![](img/Pasted%20image%2020240806083345.png)

如果我们增加如下两级索引，那么它搜索次数就变成了3次

![](img/Pasted%20image%2020240806083352.png)
...
![](img/Pasted%20image%2020240806083434.png)
...
## 6.4 ZSkipList的元素动态添加过程（ChatGPT）
当然，以下是一个具体例子，演示 Redis 跳跃表（ZSkipList）从插入一个元素到多个元素时，内部结构的动态变化情况。
...
跳跃表中只有一个元素 `A`，由于没有其他元素，`A` 仅存在于最低层（Level 0）。
...
插入第四个元素 `D`，分数为 `25`。假设随机算法决定 `D` 需要出现在 Level 1 和 Level 0。

```
Level 3:    NIL
Level 2:    NIL
Level 1:    D(25)
Level 0:    A(10) -> B(20) -> D(25) -> C(30)
```

此时，元素 `D` 在 Level 0 和 Level 1 出现，且按分数顺序插入。

+ 插入第五个元素

插入第五个元素 `E`，分数为 `15`。假设随机算法决定 `E` 需要出现在 Level 2 和以下各层。

```
Level 3:    NIL
Level 2:    E(15)
Level 1:    E(15) -> D(25)
Level 0:    A(10) -> E(15) -> B(20) -> D(25) -> C(30)
```

此时，元素 `E` 在 Level 0、Level 1 和 Level 2 出现，按分数顺序插入。

+ 插入第六个元素

插入第六个元素 `F`，分数为 `5`。假设随机算法决定 `F` 需要出现在所有层。

```
Level 3:    F(5)
Level 2:    F(5) -> E(15)
Level 1:    F(5) -> E(15) -> D(25)
Level 0:    F(5) -> A(10) -> E(15) -> B(20) -> D(25) -> C(30)
```

此时，元素 `F` 在所有层都出现，成为每一层的第一个元素。

+ 总结

在每次插入操作中，跳跃表会动态调整其结构，以确保元素按分数排序，并且在不同层级间设置索引以优化查找性能。通过这个例子，可以看到跳跃表如何在插入多个元素时，从底层到高层逐步建立索引，使得查找和范围查询操作更加高效。