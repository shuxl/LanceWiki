+ 类生命周期一共有五个阶段，但是解析阶段比较特殊
+ 类加载器可以在某个类使用前预加载。如果出现异常，只会在首次使用时才报错，否则不报错
+ 准备阶段会分配类的静态变量，但是赋值却是初始化的时候去做的 
	> 这时候进行内存分配的仅包括类变量(`static`)，而不包括实例变量，实例变量会在对象实例化时随着对象一块分配在Java堆中
+ 类初始化的时机 [1.3 初始化](#1.3%20初始化)
+ JVM类加载机制可以使用双亲委派原则 （[3 JVM类加载机制](#3%20JVM类加载机制)）
# 1 类的生命周期
...
![](img/Pasted%20image%2020240713085401.png)
...
![](img/Pasted%20image%2020240713095102.png)
